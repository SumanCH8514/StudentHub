...
const AdminUpdates = () => {
    ...
    const handleAddUpdate = async (e) => {
        ...
        try {
            ...
            setNewUpdate({ title: "", description: "", type: "notification" });
        } catch (error) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm h-fit">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="font-bold text-slate-800 dark:text-white text-lg">New Update</h3>
                    </div>
                    ...
                    <form onSubmit={handleAddUpdate} className="space-y-4">
                        ...
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-500/20 active:scale-95 disabled:opacity-70"
                        >
                            {isSubmitting ? (
                                <>
                    ...
                    <div className="p-6 flex-1 overflow-y-auto custom-scrollbar bg-slate-50/30 dark:bg-slate-900/20">
                        {loading ? (
                            ...
                            <div className="flex flex-col gap-4">
                                {updates.map((update) => (
                                    <div key={update.id} className="group flex items-start justify-between gap-4 p-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl hover:border-indigo-300 dark:hover:border-indigo-600/50 transition-all hover:shadow-md">
                                        ...
                                        <button
                                            ...
                                            className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-all opacity-0 group-hover:opacity-100 flex-shrink-0 border border-transparent hover:border-rose-100 dark:hover:border-rose-900/30"
                                            title="Delete Update"
                                        >
                                            <Trash2 size={16} />
                                        </button>
